...
- **Always verify IP-to-cluster mapping against CLUSTERS.md**: When an alert shows a node IP, CHECK CLUSTERS.md before naming the cluster. Do not guess. Cluster LAN ranges: Robbinsdale=192.168.50.0/24, StPetersburg=192.168.73.0/24, Ottawa=192.168.169.0/24. Specific trap: 192.168.73.206 is St. Petersburg (spark-0), NOT Ottawa — Ottawa is 192.168.169.0/24. Misidentifying this IP during incident analysis led to misrouted debugging.
...
- **Robbinsdale multi-node instability — kubelet API lease loss**: Stone and tank nodes oscillate NotReady/cordoned repeatedly. kubelet loses API server heartbeat → `node.kubernetes.io/unschedulable` taint re-applied every few minutes. Titan is stable. Affected nodes need physical networking investigation. Cluster can survive on remaining nodes during instability. After cluster recovers, expect transient alert cascade (immich, jellystat, jellyseerr) — all resolve on their own, no action needed.
- **Post-outage alert cascade is normal**: After Robbinsdale cluster instability, gatus fires delayed alerts for immich-server, jellystat, jellyseerr. These are deferred notifications from downtime, not new issues — all resolve after cluster stabilizes. Heartbeat endpoint also fails because gatus pod itself was affected.
- **Jellyfin deployment references deleted SMB PVCs**: When SMB shares (unas, unas-2) are renamed or deleted, the Jellyfin deployment still references them. Fix: remove the stale volume and volumeMount entries from the deployment.yaml in git.
