...
| `.github/renovate.json` Docker rule: `kubernetes/apps/base/mimir/mimir-ottawa/rules/tests/bhaiya_workspace_image_test.yaml` | This is a `promtool` fixture. Its `input_series`, expected alert labels, and annotations deliberately repeat the same workspace version; Renovate sees the label as a Docker-like version and changing only the expected label breaks the test. | Make the fixture version derive from one source value, or add a fixture-aware Renovate manager that updates all repeated test data together. | 2026-10-10 |
| `.github/renovate.json` Docker rule: `kubernetes/apps/base/mimir/mimir-ottawa/rules/tests/running_workload_images_test.yaml` | This is a `promtool` fixture. Its `input_series`, expected alert labels, and annotations repeat internal registry image strings; Renovate sees the expected `image:` labels as Docker dependencies, but these are test data rather than deployed images. | Make the fixture image values derive from one source value, or add a fixture-aware Renovate manager that updates all repeated input and expected data together. | 2026-10-10 |

The Agent Sandbox row is currently a legitimate hold, not a claim that
`v0.5.4` is current. The next owner should either return the bundle to
Renovate with that generator/test or record a new, dated decision before this
date expires.
