...
| `.github/renovate.json` Docker rule: `kubernetes/apps/base/mimir/*/rules/tests/**` | These are `promtool` fixture directories. Their `input_series`, expected alert labels, and annotations are test data rather than deployed images; the directory is not included by the Mimir production kustomization. | Keep deployed manifests outside `rules/tests/**`; if a fixture-aware Renovate manager is added that updates repeated input and expected data atomically, remove this blanket exclusion. | 2026-10-10 |
